...
**Review build: adapter 0.3.0; shared foundation 0.2.0.** This is a finite
...
| `pal24_review_resume` | Pointwise recovered work, source content and current declared requirements, grant and resources kept separate. |
| `pal24_review_resources` | Finite nested costs with unique attribution and explicit completeness; unknown costs remain unknown. |
| `pal24_source_profile_resource` | PAL v2.4 artifact pins, open obligations and finite implementation limits. |
...
| `compatibility_profile_resource` | PAL 2.4 mapping by default; `pal_version="2.3"` preserves the previous profile and PPP 0.6's original PAL 2.2 declaration. |
...
The active source references are PAL 2.4, CHARTER 1.0, PECAN 1.0.4, PEA Core 1.1.3,
SEED 0.3 and PPP 0.6. PPP's PAL 2.2 dependency and the original preview's PAL
2.2 target remain historical facts. The PAL 2.3 account checker, exact source
read and compatibility mapping remain available. PAL 2.4's [new profiles](docs/PAL24_PROFILE.md)
are additive, with [pinned sources](docs/PAL24_SOURCES.md) and O63-O65 still OPEN.
Native transport profiles and mathematical
